...
import { addTodo, fetchTasks, updateTask, deleteTasks, addToCartApi } from '../api/todoApi';
...
const TodoApp = ({handleTaskClick}) => {
  ...
  const [user, setUser] = useState({}); // Default limit per page
  const [cart, setCart] = useState([]); // Default limit per page
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-2xl w-full bg-white p-8 rounded-lg shadow-lg">
        <h1 className="text-3xl font-semibold text-center text-blue-600 mb-6">
          {editingTask ? 'Edit Product' : 'Product List'}
        </h1>
        ...
        <div className="mb-6">
          <button
            ...
            className="w-full p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600"
          >
            {isSelecting ? 'Cancel Selection' : 'Select Product'}
          </button>
        </div>
